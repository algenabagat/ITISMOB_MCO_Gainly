...
class PlannedWorkoutTrackingActivity : AppCompatActivity() {
    ...
    private var planIdToDelete: String? = null
    ...
    private fun initViews() {
        ...
        cancelWorkoutBtn.setOnClickListener {
            Toast.makeText(this, "Workout cancelled! Plan not removed.", Toast.LENGTH_SHORT).show()
            finish()
        }
    }
    ...
    private fun setupWorkout() {
        ...
        planIdToDelete = intent.getStringExtra("plan_id_to_delete")
    ...
    private fun deletePlanAfterCompletion() {
        planIdToDelete?.let { id ->
            val plans = PlanStorageManager.loadPlans(this).toMutableList()
            val planToRemove = plans.find { it.id == id }

            if (planToRemove != null) {
                plans.remove(planToRemove)
                PlanStorageManager.savePlans(this, plans)
                NotificationHelper(this).cancelNotification(planToRemove)
                Toast.makeText(this, "Completed plan removed from schedule.", Toast.LENGTH_LONG).show()
            }
        }
    }
